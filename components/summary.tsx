...
export default function Summary({ data, selectedAccount = "all" }: SummaryProps) {
  // Add a default empty object if data is not provided
  const safeData: PortfolioData = data || {
    accounts: [],
    currentHoldings: [],
    closedPositions: [],
    timeSeriesData: [],
  ...
  const { positions: positionsMetrics, deposits: depositsMetrics } = metricsCalculator(safeData, selectedAccount)
  ...
  }, [safeData, selectedAccount, positionsMetrics, depositsMetrics])
  ...
  const processOpenDeposits = (transactions: Transaction[]): ProcessedHolding[] => {
    ...
    symbolAccountGroups.forEach((transactions: Transaction[], key: string) => {
      ...
      let price = 0
      if (safeData.latestQuotes && safeData.latestQuotes.has(exactSymbol)) {
        const quote = safeData.latestQuotes.get(exactSymbol)
        price = quote ? Number(quote.Close) : 0
      } else if (safeData.quotes && Array.isArray(safeData.quotes)) {
        const matchingQuotes = safeData.quotes.filter(
  ...
  const unrealizedGainZr4_6 = (() => {
    if (!safeData || !safeData.transactions) return 0

    // Filter transactions for "Open Deposits" status and selected account
    const openDepositsTransactions = safeData.transactions.filter(
      (t) => t["Status Tr"] === "Open Deposits" && (selectedAccount === "all" || t.Account === selectedAccount),
    )
    ...
    symbolAccountGroups.forEach((transactions, key) => {
      ...
      if (safeData.latestQuotes && safeData.latestQuotes.has(exactSymbol)) {
        price = Number.parseFloat(String(safeData.latestQuotes.get(exactSymbol)!.Close))
      } else if (safeData.quotes && Array.isArray(safeData.quotes)) {
        const matchingQuotes = safeData.quotes.filter(
        ...
        if (matchingQuotes.length > 0) {
          ...
          const similarQuotes = safeData.quotes.filter(
            (q) =>
  ...
  const totalValueDepositsZr1_5 = (() => {
    ...
    const ahlyBankCash =
      selectedAccount === "all" || selectedAccount === "AhlyBank"
        ? safeData.accounts?.find((acc) => acc.name === "AhlyBank")?.summaryMetrics?.cashBalance || 0
        : 0
